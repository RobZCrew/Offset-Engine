...
import funkin.menus.ui.HelperText;
import funkin.options.ui.Option;
...
class OptionsMenu extends MusicBeatState {
    ...
    override function create() {
        ...
        add(new HelperText('Press F for help'));
    ...
    function setupGameplayTab() {
        ...
        optionsStuff.push('Shaders');
        optionsBools.push((Options.shaders ? '< ON >' : '< OFF >'));
    ...
    function setupUETab() {
        ...
        #if DISCORD_ALLOWED
        optionsStuff.push('Discord RPC');
        optionsBools.push((Options.discordRPC ? '< ON >' : '< OFF >'));
        #else
        optionsStuff.push('Discord RPC is not available or is not allowed');
        #end
        ...
        if (controls.ACCEPT) {
            switch(curTab) {
                ...
                case 2:
                    switch(curSelected) {
                         case 0:
                             changeAndSave('discordRPC');
                             updateText('discordRPC', curSelected);
                    }
            }
        }
