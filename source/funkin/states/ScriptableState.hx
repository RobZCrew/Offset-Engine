...
class ScriptableState extends MusicBeatState {
    ...
    private static var folder(default, null):Null<String> = ScriptingUtil.getFolder('states');
    ...
    override function openSubState(sub:FlxSubState) {
        var name = ScriptingUtil.getClassName(sub);
        var result = callScript('onSubstateOpen', [name]);
        ...
        script.set('state', this); // Avoid bugs
    }

    override function closeSubState() {
        var result = callScript('onSubstateClose');
    ...
    private function loadScript() {
        ...
        var path = Mods.getPath(folder + scriptName + '.hx');
